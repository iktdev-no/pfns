...
@Component
@Order(1)
class ApiAuthorizationInterceptor(
    ...
    override fun getAuthenticationMode(
        ...
        val validation = try {
            if (handler is HandlerMethod)
                handler.method.getAnnotation(RequiresApiAuthentication::class.java)
            else null
        } catch (e: Exception) {
            e.printStackTrace()
...
annotation class RequiresApiAuthentication(val mode: Mode = Mode.Strict)
